...
# who received hormonal treatment have different 
# recurrence-free survival outcomes compared to those who did not.

# Assign new variable from hormon variable to Expose to hormonal treatment and controls to Non-Expose 
gbsg$hor_t =ifelse(gbsg$hormon== "0","Non-Expose","Expose to hormonal treatment")
gbsg$hor_t =factor(gbsg$hor_t)
gbsg$hor_t <- relevel(gbsg$hor_t, ref = "Non-Expose")
...
# Create table to compare baseline characteristics of expose and non-expose to hormone treatment

column_variable <- c("age","meno","grade", "size", "nodes", "pgr","er")
tab <- CreateTableOne(vars = column_variable,
               strata = c("hor_t"), data = gbsg, factorVars = c("grade","meno"))
print(tab, nonnormal = columns_num, formatOptions = list(big.mark = ","))

# Two factors (age, menopause) are significantly different between 
# Expose to hormonal treatment and non-expose group 

##Survival analysis

## Kaplan-Meier Curve
# Visualize the survival curves for the "Expose to hormonal treatment" and "Non-Expose" groups.

fit <- survfit(Surv(rfsyear, status) ~ hor_t, data = gbsg)
ggsurvplot(fit, data = gbsg, title = "Recurrence or Death",pval = TRUE, 
            xlab = "Time in years", break.x.by = 2,
           legend = c(0.3, 0.4), legend.title = "", 
           legend.lab = c("Expose to hormonal treatment", "Non-Expose")
           )
